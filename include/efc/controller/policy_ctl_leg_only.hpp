#ifndef POLICY_CTR_LEG_ONLY_HPP
#define POLICY_CTR_LEG_ONLY_HPP
...
class PolicyCtrLegOnly : public PolicyControllerBase {
 public:
  using Ptr = std::shared_ptr<PolicyCtrLegOnly>;

  PolicyCtrLegOnly() = delete;
  ~PolicyCtrLegOnly() = default;

  PolicyCtrLegOnly(RobotBase<float>::RobotPtr robot, YAML::Node const& config)
