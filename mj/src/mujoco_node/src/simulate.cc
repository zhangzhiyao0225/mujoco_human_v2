...
void UpdateInfoText(mj::Simulate* sim, const mjModel* m, const mjData* d,
  ...
  // add Energy if enabled
  {
    ...
    if (mjENABLED(mjENBL_ISLAND) && d->nisland > 0) {
      mju::sprintf_arr(tmp, "\n%d", d->nisland);
      mju::strcat_arr(content, tmp);
      mju::strcat_arr(title, "\nIslands");
    }
...
void AlignAndScaleView(mj::Simulate* sim, const mjModel* m) {
  // Note: cameraid was removed in newer MuJoCo versions
...
void Simulate::Sync(bool state_only) {
  ...
  if (pending_.reset) {
    mj_resetData(m_, d_);
    if (reload_key_id >= 0) {
      mj_resetDataKeyframe(m_, d_, reload_key_id);
    }
    mj_forward(m_, d_);
    load_error[0] = '\0';
  ...
  if (!is_passive_) {
    ...
    if (state_only) {
      ...
      mj_copyData(d_passive_, m_passive_, d_);
    }
...
void Simulate::LoadOnRenderThread() {
  ...
  reload_key_id = mj_name2id(this->m_, mjOBJ_KEY, "reload_pose");
  if (reload_key_id >= 0) {
    this->key = reload_key_id;
    mj_resetDataKeyframe(this->m_, this->d_, reload_key_id);
