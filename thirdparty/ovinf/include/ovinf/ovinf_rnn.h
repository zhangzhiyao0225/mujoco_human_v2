...
class RnnPolicy : public BasePolicy<float> {
 ...
 public:
  ...
  RnnPolicy(const YAML::Node &config);
 ...
 private:
  ...
  // RNN hidden state
  ov::Output<const ov::Node> hidden_info_;
  ov::Output<const ov::Node> cell_info_;
  size_t hidden_size_;
  VectorT hidden_states_;
  VectorT cell_states_;
