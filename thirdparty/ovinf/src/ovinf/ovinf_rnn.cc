...
RnnPolicy::RnnPolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  input_info_ = compiled_model_.input(0);
  hidden_info_ = compiled_model_.input(1);
  cell_info_ = compiled_model_.input(2);

  hidden_size_ = hidden_info_.get_shape()[2];
  std::cout << hidden_info_ << std::endl;
  std::cin.get();

  hidden_states_ = VectorT(hidden_size_).setZero();
  cell_states_ = VectorT(hidden_size_).setZero();

  inference_done_.store(true);
  exiting_.store(false);
  worker_thread_ = std::thread(&RnnPolicy::WorkerThread, this);
}

bool RnnPolicy::WarmUp(RobotObservation<float> const &obs_pack) {
  double gait_time_value = 0.0;

  VectorT obs(single_obs_size_);
  obs.setZero();
  hidden_states_.setZero();
  cell_states_.setZero();
  gait_start_ = false;
  ...
  if (!inference_done_.load()) {
    ...
    ov::Tensor hidden_tensor(hidden_info_.get_element_type(),
                             hidden_info_.get_shape(), hidden_states_.data());
    ov::Tensor cell_tensor(cell_info_.get_element_type(),
                           cell_info_.get_shape(), cell_states_.data());

    infer_request_.set_input_tensor(0, input_tensor);
    infer_request_.set_input_tensor(1, hidden_tensor);
    infer_request_.set_input_tensor(2, cell_tensor);
...
bool RnnPolicy::InferUnsync(RobotObservation<float> const &obs_pack) {
...
std::optional<RnnPolicy::VectorT> RnnPolicy::GetResult(const size_t timeout) {
  if (inference_done_.load()) {
...
void RnnPolicy::WorkerThread() {
  ...
  while (exiting_.load() == false) {
    if (!inference_done_.load()) {
      ...
      auto hidden_tensor = infer_request_.get_output_tensor(1);
      auto cell_tensor = infer_request_.get_output_tensor(2);
      ...
      hidden_states_ =
          Eigen::Map<VectorT>(hidden_tensor.data<float>(), hidden_size_);
      cell_states_ =
          Eigen::Map<VectorT>(cell_tensor.data<float>(), hidden_size_);
    }
    inference_done_.store(true);
...
void RnnPolicy::CreateLog(YAML::Node const &config) {
...
void RnnPolicy::WriteLog(RobotObservation<float> const &obs_pack) {
